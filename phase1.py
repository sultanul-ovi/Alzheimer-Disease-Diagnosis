...
def create_model(name: str, num_classes: int = 4, pretrained: bool = True) -> nn.Module:
    ...
    if name == 'mobilenetv3_small':
        m = models.mobilenet_v3_small(weights=models.MobileNet_V3_Small_Weights.IMAGENET1K_V2 if pretrained else None)
        m.classifier = nn.Sequential(
            nn.Dropout(0.3),
            nn.Linear(m.classifier[3].in_features, 512),
            nn.ReLU(inplace=True),
...
def train_model(name, loaders, num_epochs=NUM_EPOCHS, lr=LR_BACKBONE, weight_decay=WEIGHT_DECAY):
    ...
    history = {'train_loss': [], 'train_acc': [], 'val_loss': [], 'val_acc': [], 'test_loss': [], 'test_acc': [], 'test_f1_macro': [], 'test_precision_macro': [], 'test_recall_macro': [], 'test_auc_macro': []}
    best_acc = 0.0
    best_state = None
    epochs_no_improve = 0
    ...
    for epoch in range(num_epochs):
        ...
        history['test_f1_macro'].append(te_f1)
    ...
    print(f"Final Test (with TTA): acc={te_acc:.4f} f1={te_f1:.4f} prec={te_precision:.4f} rec={te_recall:.4f} auc_macro={auc_macro:.4f}")
